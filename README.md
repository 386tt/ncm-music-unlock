...
- 🐛 **QQ音乐元信息修复** — 修正搜索 API 字段映射，补齐年份、流派、发行方、音轨/碟号等缺失字段
- 🖼️ **QQ音乐封面代理** — 封面图通过本地服务器代理加载，解决 CDN 防盗链导致封面不显示的问题
- 🎵 **QQ音乐专辑 API** — 新增专辑详情 API 调用，提取 genre、company、aDate 等完整元信息
- ⚠️ **MusicBrainz 访问提示** — 搜索结果页新增中国大陆地区可能无法访问 MusicBrainz 的提示
- 🔧 **图片代理 Referer 增强** — 自动为不同图片源添加合适的 Referer 头以绕过防盗链
